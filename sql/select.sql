...
FROM
    `default`.products p
JOIN
    `default`.sales s ON p.product_id = s.product_id
GROUP BY
...
FROM
    `default`.customers c
JOIN
    `default`.sales s ON c.customer_id = s.customer_id
GROUP BY
...
FROM
    `default`.sales s
JOIN
    `default`.dates d ON s.date_id = d.date_id
GROUP BY
    d.year, d.month_name
ORDER BY
    d.year, d.month_name;
...
WITH Categories AS (
    SELECT DISTINCT category FROM `default`.products
),
CustomerCategories AS (
    ...
    JOIN
        `default`.sales s ON c.customer_id = s.customer_id
    JOIN
        `default`.products p ON s.product_id = p.product_id
    GROUP BY
